2018.11.28:
定位模式下，如果坐标丢失，a) 可以用关键距离最近的关键帧的位置代替 b)可以用码盘里程计来更新得到的位置

2018.11.23: 测试记录
1. 建图的时候，动作要稳，并且速度要慢（不然会跟丢），各个方向要尽可能多的采集关键帧。
2. 定位的时候，可能会跟丢，可以与小车码盘里程计进行KF融合定位。
3. 摄像头需要广角的。
4. 跟丢是因为，有些地方特征点不那么多，所以视觉跟踪，环境影响还是挺大的，就目前来看，环境中角点较多的话，ORB特征比较明显。
